...
public interface ICartService extends IService<CartEntity> {
     ...
     * 加入购物车
     */
    FwResult addToCart(Integer productId);//前端传商品Id进来,然后解析请求头Token 获取userId 加入购物车的User_id
     ...
     * 修改商品数量
     */
    FwResult updateQuantity( CartUpdateParam param);//解析请求头携带的token 然后获取token里面的userId
  //修改该用户商品数量
    /**
     * 移除购物车商品
     */
    FwResult removeFromCart( Integer cartId);//解析请求头携带的token 然后获取token里面的userId
    //移除该用户的购物车商品
    /**
     * 清空购物车
     */
