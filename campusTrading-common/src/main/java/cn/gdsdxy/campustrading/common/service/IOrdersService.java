...
// 原有代码基础上添加
     ...
     * 创建订单
     */
    Integer createOrder( OrderCreateParam param);//返回Integer的含义：大概率是返回订单的数据库自增主键 ID（订单表的order_id，Integer 类型）—— 这是最常见的设计，因为创建订单后，前端需要这个
     ...
     * 订单列表（买家视角）
     */
    IPage<OrderListVo> getBuyerOrderList(Integer status, Integer pageNum, Integer pageSize);

    /**
     * 订单列表（卖家视角）
     */
    IPage<OrderListVo> getSellerOrderList(Integer status, Integer pageNum, Integer pageSize);
