...
public class UserProductController {
    ...
    // 我的发布（必加）
    @GetMapping("/my/list")
    public FwResult getMyProducts(
            @RequestParam(defaultValue = "1") Integer pageNum,
            @RequestParam(defaultValue = "10") Integer pageSize) {
        IPage<ProductListVo> page = iProductsService.getMyProducts(pageNum, pageSize);
    ...
    public FwResult<String> generateDesc(@RequestParam String productName) {
        String desc = doubaoService.generateProductDescription(productName);
        if (desc == null || desc.trim().isEmpty()) {
            return FwResult.fail("生成描述失败，请稍后重试");
        }
        return FwResult.ok(desc);
    }
